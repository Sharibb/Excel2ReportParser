...
ALLOWED_EXTENSIONS = {"docx", "xlsx", "xls", "zip"}
...
@app.route("/phase3")
def phase3():
    """Phase 3: Complete report generation with PoC ZIP."""
...
def phase3_generate():
    """Handle Phase 3: Generate Word document from Excel, template, and PoC ZIP."""
    try:
        # Validate file uploads
        if "excel_file" not in request.files or "template_file" not in request.files or "poc_zip" not in request.files:
            return jsonify({"error": "Excel file, template file, and PoC ZIP are required"}), 400

        excel_file = request.files["excel_file"]
        template_file = request.files["template_file"]
        poc_zip = request.files["poc_zip"]

        if excel_file.filename == "" or template_file.filename == "" or poc_zip.filename == "":
        ...
        if not allowed_file(poc_zip.filename, {"zip"}):
            return jsonify({"error": "PoC file must be .zip"}), 400

        # Save uploaded files
        excel_filename = secure_filename(excel_file.filename)
        template_filename = secure_filename(template_file.filename)
        zip_filename = secure_filename(poc_zip.filename)
        ...
        poc_zip.save(zip_path)

        # Prepare request to backend
        with open(excel_path, "rb") as ef, open(template_path, "rb") as tf, open(zip_path, "rb") as zf:
            files = {
                ...
                "poc_zip": (zip_filename, zf, "application/zip"),
            }

            response = requests.post(
                f"{BACKEND_URL}/api/phase3/generate",
                files=files,
        ...
        if response.status_code == 200:
            ...
            output_filename = Path(template_filename).stem + "_generated_with_pocs.docx"
            ...
            return jsonify({
                "success": True,
                "message": "Word document with PoCs generated successfully!",
